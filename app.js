...
async function loadAutomaticLocation() {
  ...
  elements.note.textContent = "Lokales Wetter wird automatisch geladen...";
  ...
  try {
    ...
    elements.note.textContent = `Automatisch erkannt: ${displayCityName(location.name)}.`;
  } catch (error) {
    console.error(error);
    elements.note.textContent =
      "Automatische Standorterkennung war nicht moeglich. Berlin wird angezeigt.";
    await loadCity("Berlin");
  } finally {
    setLoading(false);
  }

  loadPreciseLocation(false);
}
...
async function loadPreciseLocation(isManualRequest = false) {
  ...
  try {
    ...
    if (isManualRequest) {
      elements.note.textContent =
        "Der genaue Standort wurde nicht erlaubt. Das ungefaehre Standortwetter bleibt aktiv.";
    }
    console.error(error);
  } finally {
    setLoading(false);
...
function getSourceLabel(source) {
  ...
  if (source === "approximate") return "Automatisch erkannt";
  return "Stadt-Suche";
}
...
function buildWeatherStory(condition, temperature, rainChance, windSpeed, source) {
  ...
  const sourceText =
    source === "approximate"
      ? "Die automatische Erkennung ist ungefaehr; fuer mehr Genauigkeit den Standort erlauben."
